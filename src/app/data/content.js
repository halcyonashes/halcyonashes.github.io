const content = {
    title: "Onash Maharjan",
    summary: "Summary",
    summaryText: 
`Highly motivated software engineer with a proven track record of successfully delivering multiple projects, both in a team and as an individual. 
Proficiency in: Kotlin, Java, Flutter, and Android SDK for Application Development.`,
    experience: "Experience",
    job1: {
      title: "Graduate Application Developer, University of Exeter",
      description: 
`• Working on the university app using Flutter.
• Transformed a web app that used Tailwind CSS to vanilla CSS.
• End-to-end testing, creating technical documents for releases, demoing the product to stakeholders.
• Worked on a 2D game using Unity for showcase in a digital conference.`,
      years: "06/2023 - Present"
    },
    job2: {
      title: "Dispensing Operative (Part-time), Numan Operations Limited",
      description: "• Processing prescription data and dispensing medical supplies",
      years: "11/2021-05/2023"
    },
    job3: {
        title: "Android Developer",
        description: 
`• Worked closely with a stakeholder to develop and maintain data auditing application used for healthcare research.
• Chat application using an open-source platform (RocketChat).
• Published multiple entrance examination preparation applications to the Play Store.`,
        years: "01/2019 - 04/2020"
      },
      job4: {
        title: "Android Developer",
        description: 
`• Event management system and hotel management applications.
• Implemented ExoPlayer and Calendars for a priest application.`,
        years: "06/2017 - 07/2018"
      },
    projects: "Projects",
    education: "Education",
    degree1: {
      title: "MSc. Data Science",
      institution: "Cardiff Metropolitan University, 2022"
    },
    degree2: {
      title: "BE Computer Engineering",
      institution: "Tribhuvan University, 2019"
    },
    skills: "Skills",
    skillsText: "Application Development | Kotlin | Java | Flutter | Python for Data Science | SQL database management | AWS Services | C | C++ | Django | Agile methodologies | Jira | Git Version Control | Data Visualisation | ReactJS | PHP | JavaScript | CSS | HTML5 | Postman | Wordpress",
    languages: "Languages",
    languageList1: ["Nepali - Native", "English - Fluent", "Spanish - Beginner"],
    languageList2: ["Newari - Native", "Hindi - Expert"],
    researchInterests: "Research Interests",
    researchInterestsText: "Application Development | Natural Language Processing | Artificial Intelligence | Data Analytics",
    linkedinUrl: "https://www.linkedin.com/in/halcyonashes",
    githubUrl: "https://github.com/halcyonashes"
  };
  
  export default content;
  